...
nelson_siegel_curve <- function(input_curve_points, 
  ...
  cli::cli_h1("Calculating Nelson-Siegel")
    ...
    output <- tibble::tibble(ls_estim = sq_sum,
                             beta_2 = beta_2,
                             lambda = lambda)
    }
    
    
    ls <- purrr::map_df(1:sample_number_per_simulation, ls_simulation)
    
    min_ls <- ls %>% 
      dplyr::filter(ls_estim == min(ls_estim))
    return(min_ls)
    
    }
    
    sim_results <- purrr::map_df(cli::cli_progress_along(name = "Simulating:", 1:100), 
                                 head_simulation)
    min_sim <- sim_results %>% 
      dplyr::filter(ls_estim == min(ls_estim))
    ...
    sim_curve <- tibble::tibble(time_step = output_curve_steps) %>% 
      dplyr::mutate(
      ...
      dplyr::select(time_step, sim_curve)
    
    
    ns_parameters <- tibble::tibble(
      beta_0 = beta_0,
      beta_1 = beta_1,
    ...
    out_list <- list(ns_params = ns_parameters,
                     curve = sim_curve) 
    return(out_list)
